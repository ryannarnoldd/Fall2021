import { GetByHandle } from require('./users');
...
modules.exports.GetAll = function GetAll() {
    return listWithOwner();
}

modules.exports.GetWall = function GetWall(handle) {
    return listWithOwner().filter(post=> post.user_handle == handle);
}

modules.exports.GetFeed = function GetFeed(handle) { return listWithOwner()
    .filter(post=> GetByHandle(handle).following.some(f=> f.handle == post.user_handle && f.isApproved) );     }


modules.exports.Get = function Get(post_id) { return list[post_id]; }
modules.exports.Add = function Add(post) {
    if(!post.user_handle){
...
modules.exports.Update = function Update(post_id, post) {
    const oldObj = list[post_id];
    const newObj = { ...oldObj, ...post }
    list[post_id] = newObj ;
    return newObj;
}
modules.exports.Delete = function Delete(post_id) {
